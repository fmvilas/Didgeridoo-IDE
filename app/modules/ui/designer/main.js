	...
	var Designer = function(documentId) {
	    	...
	    	var _dom2box = function(element) {
		    	...
		    	if(element.nodeType === 1) {
		    		...
		    		box.addEventListener('scroll', function(ev) {
		    			$(ev.srcElement).data('originalElement').scrollTop = ev.srcElement.scrollTop;
						$(ev.srcElement).data('originalElement').scrollLeft = ev.srcElement.scrollLeft;
	    ...
	    var Interaction = (function() {
	    	...
	    	var _selectElement = function(element, force, silent) {
	    		if( force || (element !== null && element.classList.contains('element')) ) {
		    	...
		    	console.dir($(selectedElement).data('originalElement'));    	
	    	...
	    	var _dblclickEventHandler = function(ev) {
	    		if( !editing && !resizing && ev.target.classList.contains('element') ) {
					_makeBoxResizable(ev.srcElement);
				}
	    	...
	    	var _scrollEventHandler = function(ev, force) {
				if(force || ev.srcElement === handler.document) {
		...
		var _renderTo = function(selector, callback) {
			...
			assert(	typeof selector === 'string' ||
					...
					'Error in module ' + config.id + '.\n' +
					'Designer._renderTo(selector[, callback]): "selector" must be a String or an Object.');
			
			assert(	typeof callback === 'function' ||
					typeof callback === 'undefined',
					'Error in module ' + config.id + '.\n' +
					'Designer._renderTo(selector[, callback]): "callback" is not a Function.');
