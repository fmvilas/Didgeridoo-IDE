"use strict";
define(['autoGrowInput'], function() {
	
	var module,
		moduleName = 'didgeridoo-dominspector';
	...
	var DOMInspector = function() {
		...
		var _renderTo = function(selector, callback) {
			...
			$.ajax({
				...
				success: function(data) {
				    ...
				    $('#didgeridoo-dominspector .didgeridoo-panel-window-header').click(function() {
				    	if($('#didgeridoo-dominspector').hasClass('didgeridoo-panel-window-collapsed')) {
					    	$('#didgeridoo-dominspector').removeClass('didgeridoo-panel-window-collapsed');
					    } else {
					    	$('#didgeridoo-dominspector').addClass('didgeridoo-panel-window-collapsed');
					    }
				    });
				    ...
				    $('.didgeridoo-panel-window-close', '#didgeridoo-dominspector').live('click', function(e) {
				    ...
				    $('#didgeridoo-dominspector').on('change', '.applied', function(ev) {
				    ...
				    $('#didgeridoo-dominspector').on('keydown', '.propName', function(ev) {
				    	switch(ev.keyCode) {
				    		case 13:
				    		case 186:
				    			var $valueInput,
				    ...
				    $('#didgeridoo-dominspector').on('keydown', '.propValue', function(ev) {
				    	switch(ev.keyCode) {
				    		case 13: //Enter
				    			var $nameInput,
				    				$rule = $(this).parents('.css-rule'),
				    ...
				    $('#didgeridoo-dominspector').on('blur', '.propName, .propValue', function(ev) {
				    ...
				    $('#didgeridoo-dominspector').on('click', '.ruleName', function(ev) {
		    		...
		    		didgeridoo.observer.subscribe('didgeridoo-designer.element.select', function(topic, el) {
		    			if(typeof el !== 'undefined') {
		    				...
		    				$('.didgeridoo-panel-window-content', '#didgeridoo-dominspector').empty();
		    			}
		    		});
		...
		var _update = function(el) {
			...
			var CSSRules = _getCSSRules(element),
				...
				container = $('.didgeridoo-panel-window-content', '#didgeridoo-dominspector')[0],
				html = '';
		...
		didgeridoo.observer.subscribe('didgeridoo-designer.element.change', function(topic, el) {
			_update(el[0]);
		});
